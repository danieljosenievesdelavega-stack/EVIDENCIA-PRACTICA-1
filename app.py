...
def parse_number(value, base):
    ...
    try:
        ...
        raise ValueError("Dígito inválido para la base seleccionada.") from error
...
def pad_number(value, bits, base):
    widths = {2: bits, 8: (bits + 2) // 3, 16: bits // 4}
...
def validate_binary(value):
    if not isinstance(value, str) or any(bit not in "01" for bit in value):
        raise ValueError("Las cadenas deben contener solo 0 y 1.")
...
def alu_bitwise(first, second, operation):
    ...
    return "".join("1" if operations[operation](left, right) else "0"
...
def convert():
    ...
    try:
        value = parse_number(str(data.get("number", "")), int(data.get("base", 10)))
        bits = int(data.get("bits", 8))
        if bits not in SUPPORTED_WORD_SIZES:
            raise ValueError("El tamaño de palabra no es válido.")

        maximum = (2 ** bits) - 1
        if value > maximum:
            raise ValueError(
                f"Overflow / Desbordamiento de Registro. Máximo permitido en {bits} bits es {maximum}"
            )

        return jsonify({
            "bin": pad_number(format_number(value, 2), bits, 2),
            "oct": pad_number(format_number(value, 8), bits, 8),
            "dec": str(value),
            "hex": pad_number(format_number(value, 16), bits, 16),
        })
